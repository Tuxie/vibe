...
- Forbidden commands: `install`, `add`, `build`, `migrate`, `exec`, `test`, `run`, any package-manager subcommand that modifies project state (lockfile, node_modules, virtualenv), and any execution of project code. Allowed: `git log`, `git blame`, `git ls-files`, `rg`, `ls`, `wc`, the Read tool, and (for dependency-freshness checks only) the read-only outdated-command allowlist in `analyst-ground-rules.md` §6: `bun outdated`, `npm outdated`, `pnpm outdated`, `yarn outdated`, `cargo outdated`, `pip list --outdated`, `uv pip list --outdated`, `gem outdated`, `go list -m -u all`, `mix hex.outdated`, `composer outdated`. Web-search (if available) is preferred when the native command is absent.
